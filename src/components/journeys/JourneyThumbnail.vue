<template>
  <div class="flex my-5"> 
      <div class="bg-white shadow-md border rounded-3xl p-4">
        <div class="flex-none lg:flex">
          <div class="h-full w-full lg:h-48 lg:w-48 lg:mb-0 mb-3">
            <img @click="journeySelected(journeyData)" :src="journeyData.image"
              :alt="journeyData.name" class=" w-full object-cover h-48 rounded-2xl">
          </div>
          <div class="flex-auto ml-3 justify-evenly py-2">
            <div class="flex flex-wrap ">
              <div class="w-full flex-none text-xs text-blue-700 font-medium ">
                {{ journeyData.category }}
              </div>
              <h2 class="flex-auto text-lg font-light">{{ journeyData.name }}</h2>
            </div>
            <p class="mt-3"></p>
          ...
          <div class="flex space-x-3 text-sm font-medium">
            ...
            <button class="mb-2 md:mb-0 bg-primary px-5 py-2 shadow-sm tracking-wider text-white rounded-full hover:bg-secondary" type="button" aria-label="like">View Roadmaps</button>
          </div>
        </div>
      </div>
    </div>
